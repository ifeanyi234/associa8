          ...
          <section class="dashboard-card structure-form-card">
            <form action="proc-add-cbt-question.php" method="POST" class="structure-form">
              ...
              <div class("form-group")>
                ...
                <textarea class("form-control") id("question") name("question_text") rows("4") required></textarea>
              </div>
              <div class("form-row-2col")>
                <?php foreach (['a','b','c','d','e'] as $option): ?>
                  <div class("form-group")>
                    <label class("form-label") for("option<?php echo strtoupper($option); ?>")>Option <?php echo strtoupper($option); ?><?php echo $option === 'e' ? ' (optional)' : ''; ?></label>
              ...
              <div class("structure-form-actions"))
                ...
                <button class("btn-navy-filled") type("submit"))><i class("fa-solid fa-check")).></i> Save question</button)
              </div)
            </form)
          </section)
        </div)
