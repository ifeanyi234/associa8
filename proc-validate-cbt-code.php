...
$statement = mysqli_prepare($conn, "SELECT a.id, a.org_id, a.applicant_name, a.email, a.exam_code, a.exam_expires_at, a.cbt_exam_id, e.title AS exam_title, e.duration_minutes FROM admissions a INNER JOIN cbt_exams e ON e.id = a.cbt_exam_id AND e.org_id = a.org_id WHERE LOWER(a.email) = ? AND a.exam_code = ? AND a.org_id IS NOT NULL AND a.status = 'cbt_scheduled' LIMIT 1");
if (!$statement) {
...
$assessmentExpiresAt = time() + ((int) $admission['duration_minutes'] * 60);
$assessmentExpiresSqlValue = date('Y-m-d H:i:s', $assessmentExpiresAt);
$startStatement = mysqli_prepare($conn, 'UPDATE admissions SET exam_expires_at = ? WHERE id = ? AND status = \'cbt_scheduled\'');
if (!$startStatement) {
    header('Location: cbt-code-entry.php?status=error&msg=' . urlencode('The assessment could not be started.'));
    exit;
}
mysqli_stmt_bind_param($startStatement, 'si', $assessmentExpiresSqlValue, $admission['id']);
if (!mysqli_stmt_execute($startStatement)) {
    header('Location: cbt-code-entry.php?status=error&msg=' . urlencode('The assessment could not be started.'));
    exit;
}
...
$_SESSION['applicant_exam_expires_at'] = $assessmentExpiresSqlValue;
